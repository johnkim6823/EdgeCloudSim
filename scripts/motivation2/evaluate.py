                    ...
                    for i, part in enumerate(parts):
                        if part == 'SCENARIO':
                            for j in range(i + 1, len(parts)):
                                if 'DEVICES' in parts[j]:
                                    policy_name = '_'.join(parts[i + 1:j])
                                    break
                            if j + 1 < len(parts):
                                category_name = '_'.join(parts[j + 1:]).replace('.log', '')
                                break
    ...
    for file_name in os.listdir(base_dir):
        if file_name.endswith('.tar.gz') or file_name.endswith('.log'):
            try:
                ...
                print(f"Error copying or extracting file {file_name}: {e}")
...
def remove_nested_ite_folders(ite_dirs):
    for ite_dir in ite_dirs:
        try:
            ...
            print(f"Error removing nested ite folder {nested_ite_dir}: {e}")
...
def remove_progress_folder(date_dir):
    try:
        ...
        print(f"Error removing progress folder {progress_dir}: {e}")
